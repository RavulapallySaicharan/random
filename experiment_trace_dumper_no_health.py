#!/usr/bin/env python3
"""
Experiment Trace Dumper (No Health Check)

This script dumps all traces from a specific MLflow experiment without requiring a health endpoint.
It's designed to work with MLflow instances that don't expose a /health endpoint.

Usage:
    python experiment_trace_dumper_no_health.py --mlflow_url <base_url> --experiment_id <experiment_id> [options]

Example:
    python experiment_trace_dumper_no_health.py --mlflow_url https://ssds-dev-ingress.statestr.com/ssds/dev/rcd/01/snoaiobservability --experiment_id 260533303499057285
"""
...
class ExperimentTraceDumperNoHealth:
    """Class to handle dumping traces from a specific MLflow experiment without health check."""
    ...
    def __init__(self, mlflow_url: str, username: Optional[str] = None, password: Optional[str] = None):
        ...
        logger.info(f"Initialized dumper for MLflow URL: {self.mlflow_url}")
...
def main():
    ...
    parser = argparse.ArgumentParser(description='Dump all traces from a specific MLflow experiment (no health check)')
    ...
    try:
        ...
        dumper = ExperimentTraceDumperNoHealth(
